...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template
		match="change:block[@type = 'modules_markergmaps_position' or @type = 'modules_markergmaps_folder']">
		<change:block>
			<xsl:for-each select="@*">
				<xsl:choose>
					<xsl:when test="name(.) = '__width'">
						<xsl:if test=". != ''">
							<xsl:attribute name="__mapWidth"><xsl:value-of
								select="." /></xsl:attribute>
						</xsl:if>
					</xsl:when>
					<xsl:when test="name(.) = '__height'">
						<xsl:if test=". != ''">
							<xsl:attribute name="__mapHeight"><xsl:value-of
								select="." /></xsl:attribute>
						</xsl:if>
					</xsl:when>
